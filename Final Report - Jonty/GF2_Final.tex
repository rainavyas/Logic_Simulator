...
\begin{figure}[h!]
\begin{centering}
\includegraphics[scale=0.7]{{"panel_locations"}.png}
...
\begin{centering}
\begin{tabular}{c c}
  \includegraphics[scale=0.25]{{"file_error"}.png} & \includegraphics[scale=0.25]{{"file_error_GUI"}.png}\\
...
Valid File is loaded                                                                                                             & \begin{tabular}[c]{@{}l@{}}GUI loads the file and displays the stated monitoring points and switches\\ (canvas default in 3D with option to change to 2D).\end{tabular} \\ \hline
Invalid File loaded through GUI                                                                                                  & Dialogue Box appears reporting the specific semantic/syntactic error.                                                                                                   \\ \hline
\begin{tabular}[c]{@{}l@{}}Invalid File loaded through command\\ line\end{tabular}                                               & \begin{tabular}[c]{@{}l@{}}Syntactic/Semantic error message is printed to terminal and GUI does\\ not open.\end{tabular}                                                \\ \hline
Toggling between 3D and 2D views                                                                                                 & Signal traces flips between the two views.                                                                                                                              \\ \hline
Monitoring points added and removed                                                                                              & \begin{tabular}[c]{@{}l@{}}Monitoring points and signal traces update appropriately. If many\\ monitoring points are added a scrollbar appears.\end{tabular}            \\ \hline
\begin{tabular}[c]{@{}l@{}}Circuit definition file loaded with\\ large number of switches\end{tabular}                           & Scroll bar appears on switch panel and all switches are displayed.                                                                                                      \\ \hline
User enters a large number of cycles                                                                                             & \begin{tabular}[c]{@{}l@{}}Range is limited to 0-100 and the control buttons RUN and\\ CONTINUE display the cycles for the selected number of cycles.\end{tabular}      \\ \hline
User selects the EXIT button                                                                                                     & The program closes without any errors in the terminal.                                                                                                                  \\ \hline
\begin{tabular}[c]{@{}l@{}}User sets the language environment \\ variable to Hindi or French\end{tabular}                        & The GUI loads with the correct translations of all strings.                                                                                                             \\ \hline
\begin{tabular}[c]{@{}l@{}}User attempts panning of the signal\\ traces into white space\end{tabular}                            & GUI prevents panning into complete white space.                                                                                                                         \\ \hline
\begin{tabular}[c]{@{}l@{}}User loads a file and then clicks the\\ continue button before clicking the\\ run button\end{tabular} & GUI displays error message and no signal is drawn.                                                                                                                      \\ \hline
\begin{tabular}[c]{@{}l@{}}User resizes the screen to as small\\ as possible\end{tabular}                                        & GUI does not allow resizing beyond a certain size.
\end{tabular}
\end{table}
